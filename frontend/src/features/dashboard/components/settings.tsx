...
const Settings = ({ onOpen, onClose, userData }: {
    ...
    const handleSaveName = async () => {
        ...
        try {
            ...
            await axios.put("http://localhost:8000/api/v1/auth/name", { name: editName }, {
    ...
    const handleRequestPasswordOtp = async () => {
        try {
            ...
            const token = localStorage.getItem("token");
            await axios.post("http://localhost:8000/api/v1/auth/password-otp/generate", {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleVerifyPasswordOtp = async () => {
        ...
        try {
            ...
            const token = localStorage.getItem("token");
            await axios.post("http://localhost:8000/api/v1/auth/password-otp/verify", { otp: otpInput }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDeleteAccount = async () => {
        try {
            ...
            const token = localStorage.getItem("token");
            await axios.delete("http://localhost:8000/api/v1/auth/delete-account", {
                headers: { Authorization: `Bearer ${token}` }
            });
            localStorage.removeItem("token");
